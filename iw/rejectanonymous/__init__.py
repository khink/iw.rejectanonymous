...
def reject_missing_password(portal, request):
    ...
    if request.cookies.get(cookie_name):
        return
    name_password_tuple = request._authUserPW()
    if not name_password_tuple:
        show_basicauth_popup(request)
        return
    if not basicauth_validate(name_password_tuple):
        show_basicauth_popup(request)
    request.response.setCookie(cookie_name, 'content doesnt matter', path='/')
